...
const pageSize = 15;
const states = [{ Color: 'geekblue', Value: 'Pending' }, { Color: 'green', Value: 'Approved' }, { Color: 'volcano', Value: 'Rejected' }];
...
function Clients() {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_API_ENDPOINT ?? ""}/users?limit=${50}&skip=${0}&select=firstName,lastName,birthDate`)
            .then((resp) => {
                if (resp.ok) {
                    resp.json().then((data) => {
    ...
    return (
        <DefaultLayout>
            <Table columns={columns} dataSource={clients} pagination={{ pageSize: pageSize, position: ['bottomCenter'] }} />
        </DefaultLayout>
    );
}
